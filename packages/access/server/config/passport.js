  ...
  passport.use(new LocalStrategy({
    ...
    function(email, password, done) {
      User.findOne({$or: [{email: email }, {username:  email }] }, function(err, user) {
        ...
        if (!user) {
          return done(null, false, {message: 'Your email address or username is not registered.'});
        }else{
          User.find({_id : user._id, isActive:true}).exec(function(err, active) {
            if (err) {
              return done(err);
            }
            if (active.length<1) {
              return done(null, false, {message: 'Your email address or username is not activated.'});
            }else{
